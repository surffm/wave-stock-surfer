...
const getCryptoSymbol = useCallback((symbol) => {
  ...
  const fetchYahooChartData = useCallback(async (symbol) => {
  try {
    // Fetch last 24 hours with 1-minute intervals for maximum detail
    ...
    const quotes = data.chart.result?.[0]?.quote;
    
    if (!quotes || !quotes.close || quotes.close.length === 0) return null;
    
    // Return close prices, filtering out null values
    return quotes.close.filter(price => price !== null);
  } catch (error) {
...
useEffect(() => {
  const loadChartData = async () => {
    ...
    for (const stock of stocks) {
      ...
      const yahooData = await fetchYahooChartData(stock.symbol);
      
      if (yahooData && yahooData.length > 0) {
        // Use real data from Yahoo
        histories[stock.symbol] = yahooData;
      } else {
        // Fallback to generated data if Yahoo fails
        histories[stock.symbol] = stock.history;
      }
    ...
    setChartHistories(histories);
  };
  
  loadChartData();
}, [stocks, fetchYahooChartData]);
...
useEffect(() => {
 ...
 const submitScore = useCallback(async () => {
  ...
  const createCutbackSplash = useCallback((stockSymbol, x, y) => {
    ...
    const history = chartHistories[stock.symbol] || stock.history;
  ...
  }, [stocks, surferPositions, chartHistories]);
  ...
  }, [surferPositions, selectedChars, characters, selectedStock, waterTrails, cutbackSplashes, realPrices, priceChanges, chartHistories]);
